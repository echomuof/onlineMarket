...
public class Routes extends Restful {
    ...
    static void checkService(String service) {
        if(service.contains(",")) throw new IllegalStateException("api(" + service + ") contain illegale ','");
        ...
        if(interfaceName.indexOf('*') != -1 && interfaceName.indexOf('*') != interfaceName.length() -1) {
            throw new IllegalStateException("api(" + service + ") only allow 1 *, and must be last char!");
        }
    }
