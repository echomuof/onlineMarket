        ...
        if (service != null && service.length() > 0) {
            ...
            if (consumers != null && consumers.size() > 0) {
                for (URL u : consumers) {
                    ...
                    row.add(s.replace("&", "&amp;"));
                    row.add("<button onclick=\"if(confirm('Confirm unsubscribe consumer?')){window.location.href='unsubscribe.html?api=" + service + "&consumer=" + URL.encode(s) + "';}\">Unsubscribe</button>");
                    rows.add(row);
                }
            }
            return new Page("<a href=\"services.html\">Services</a> &gt; " + service 
                    + " &gt; <a href=\"providers.html?api=" + service
                    + "\">Providers</a> | Consumers | <a href=\"statistics.html?api=" + service
                    + "\">Statistics</a> | <a href=\"charts.html?api=" + service
            ...
            throw new IllegalArgumentException("Please input api or host or application parameter.");
        }
    }
