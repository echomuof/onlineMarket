    ...
    protected void checkRegistry() {
        ...
        if ((registries == null || registries.size() == 0)) {
            throw new IllegalStateException((getClass().getSimpleName().startsWith("Reference") 
                    ? "No such any registry to refer api in consumer "
                        : "No such any registry to export api in provider ")
                                                    + NetUtils.getLocalHost()
                                                    + " use dubbo version "
                                                    + Version.getVersion()
                                                    + ", Please add <dubbo:registry address=\"...\" /> to your spring config. If you want unregister, please set <dubbo:api registry=\"N/A\" />");
        }
        ...
        if (methods != null && methods.size() > 0) {
            for (MethodConfig methodBean : methods) {
                ...
                if (methodName == null || methodName.length() == 0) {
                    throw new IllegalStateException("<dubbo:method> name attribute is required! Please check: <dubbo:api interface=\"" + interfaceClass.getName() + "\" ... ><dubbo:method name=\"\" ... /></<dubbo:reference>");
                }
        ...
        if (ConfigUtils.isNotEmpty(mock)) {
            if (mock.startsWith(Constants.RETURN_PREFIX)) {
                ...
                try {
                    ...
                    throw new IllegalStateException("Illegal mock json value in <dubbo:api ... mock=\"" + mock + "\" />");
                }
            } else {
