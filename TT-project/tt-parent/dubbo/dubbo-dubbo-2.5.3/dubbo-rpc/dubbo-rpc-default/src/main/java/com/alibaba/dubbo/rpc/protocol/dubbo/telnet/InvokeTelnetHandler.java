 ...
 */
@Activate
@Help(parameter = "[api.]method(args)", summary = "Invoke the api method.", detail = "Invoke the api method.")
public class InvokeTelnetHandler implements TelnetHandler {
    ...
    public String telnet(Channel channel, String message) {
        ...
        if (service != null && service.length() > 0) {
            buf.append("Use default api " + service + ".\r\n");
        }
        int i = message.indexOf("(");
        if (i < 0 || ! message.endsWith(")")) {
            return "Invalid parameters, format: api.method(args)";
        }
        ...
        if (invoker != null) {
            if (invokeMethod != null) {
                ...
                buf.append("No such method " + method + " in api " + service);
            }
        } else {
            buf.append("No such api " + service);
        }
        return buf.toString();
    }
